...
At this stage, the repository demonstrates two complementary experimental results.

First, the MQTT-blockchain prototype validates:

- successful integration between off-chain and on-chain components  
- reliable transaction execution on a local Hyperledger Besu network  
- real-time message processing via MQTT  
- traceability of events through blockchain logs  
- preservation of confidentiality by keeping raw sensor data off-chain  

Second, the isolated ABE experimental module validates:

- encryption and decryption correctness for IIoT sensor payloads  
- measurement of cryptographic processing overhead  
- measurement of ciphertext size expansion  
- measurement of MQTT protected message overhead  
- evaluation of access policy complexity  
- evaluation of message volume using real sensor data  

The ABE experiments are available in the `abe-experiments/` directory.
...
- MQTT, ABE, and blockchain are not yet integrated into a single end-to-end execution pipeline  
- the blockchain prototype still registers metadata and hashes generated by the orchestrator  
- the ABE module is evaluated independently from the blockchain layer  
- the current experiments are limited to a local development environment  
- large-scale distributed deployments with multiple organizations and heterogeneous industrial devices remain future work  
...
- a functional MQTT-blockchain experimental baseline  
- a validated pipeline for metadata registration and auditability  
- a reproducible local environment  
- an experimental ABE module for protecting IIoT sensor payloads  
- quantitative evidence on cryptographic overhead, payload expansion, policy complexity, and real sensor message volume  
